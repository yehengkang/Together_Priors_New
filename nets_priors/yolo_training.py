...
class YOLOLoss(nn.Module):    
    def __init__(self, num_classes, strides=[8, 16, 32]):
        ...
        self.bcewithlog_loss    = nn.BCEWithLogitsLoss(reduction="none") # （二分类交叉熵）用于 分类loss
        self.iou_loss           = IOUloss(reduction="none")
        self.grids              = [torch.zeros(1)] * len(strides)

    def forward(self, inputs, labels=None, padding_masks=None):
        ...
        #            [batch_size, 6400]]
        # 注释是错误的。正确的形状应该是：
        # - x_shifts: [[1, 6400], [1, 1600], [1, 400]] （而不是 [[batch_size, xxx], ...]）
        # grid表示的是特征图上的网格坐标，这些坐标对所有batch中的图像都是相同的（都是0,1,2...79的x,y坐标），所以不需要为每个batch复制一份
        # 这样的设计是正确且高效的，因为grid坐标在所有batch间共享，只在需要时才broadcast到batch维度。
        
        # expanded_strides 是一个记录每个anchor点对应的下采样倍数（stride）的张量。
        # - 对于80×80的特征图（6400个点）：创建一个形状为 [1, 6400] 的张量，所有值都是8
        # - 对于40×40的特征图（1600个点）：创建一个形状为 [1, 1600] 的张量，所有值都是16
        # - 对于20×20的特征图（400个点）：创建一个形状为 [1, 400] 的张量，所有值都是32
        #-----------------------------------------------#
        
        # 新增：# 损失忽略padding
        padding_masks_per_level = []
    
        for k, (stride, output) in enumerate(zip(self.strides, inputs)):
            # 新增：损失忽略padding：在处理output之前获取特征图尺寸
            if padding_masks is not None:
                hsize, wsize = inputs[k].shape[-2:]  # 从原始inputs获取特征图尺寸
                # padding_masks: [batch_size, 640, 640] -> [batch_size, 1, 640, 640]
                mask_downsampled = F.interpolate(
                    padding_masks.unsqueeze(1).float(),
                    size=(hsize, wsize),
                    mode="nearest"
                )  # [batch_size, 1, hsize, wsize]

                # flatten: [batch_size, 1, hsize, wsize] -> [batch_size, hsize*wsize]
                mask_flat = mask_downsampled.view(padding_masks.shape[0], -1)
                padding_masks_per_level.append(mask_flat)
            # -------------------到这里为止----------------
        ...
        # 新增：损失忽略padding：拼接所有层的padding_masks
        if padding_masks is not None:
            padding_masks_all = torch.cat(padding_masks_per_level, dim=1)  # [batch_size, 8400]
        else:
            padding_masks_all = None
        # -------------------到这里为止----------------

        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, torch.cat(outputs, 1), padding_masks_all)
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs, padding_masks=None):
        #-----------------------------------------------#
        ...
        # loss_obj    = (self.bcewithlog_loss(obj_preds.view(-1, 1), obj_targets)).sum()  #这里无用
        # loss_obj = (self.focal_loss(obj_preds.sigmoid().view(-1, 1), obj_targets)).sum()  # 这里是原来的损失计算
        # # ----------------添加：损失忽略padding----------------
        loss_obj_raw = self.focal_loss(obj_preds.sigmoid().view(-1, 1), obj_targets)  # [batch*8400, 1]

        if padding_masks is not None:
            # padding_masks: [batch_size, 8400] -> [batch*8400, 1]
            padding_masks_flat = padding_masks.view(-1, 1)
            # 只计算非padding区域的损失（mask=1的位置）
            loss_obj = (loss_obj_raw * padding_masks_flat).sum()
        else:
            loss_obj = loss_obj_raw.sum()
        # # ----------------到这里为止----------------
