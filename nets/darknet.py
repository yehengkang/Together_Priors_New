...
#             x = self.conv(x)
#         b, _, w, h = x.shape    # torch.Size([4, 256, 20, 20])
#         p = x.flatten(2).unsqueeze(0).transpose(0, 3).squeeze(3)   # torch.Size([400, 4, 256])
...
class CSPDarknet(nn.Module):
    ...
    def forward(self, x):
        '''
        这里除了添加的部分，其余网络结果和传输顺序都和yolox一样。self.dark2-5的结构都一样。
